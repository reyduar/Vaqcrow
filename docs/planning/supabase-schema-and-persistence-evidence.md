...
- **Sin políticas RLS, hoy.** El modelo de acceso depende de que `service_role` sea el único rol con GRANT. Cualquier GRANT futuro sobre esta tabla sin política RLS abre acceso irrestricto a nivel de fila. Seguimiento: issue [#196](https://github.com/reyduar/Vaqcrow/issues/196), abierto al auditar esta Feature. El puntero original a [#134](https://github.com/reyduar/Vaqcrow/issues/134) era **equivocado**: ese issue es sobre Auth.js y límites de sesión, no sobre RLS, y estaba cerrado sin implementación. **#43 no lo investigó ni lo corrigió.**
...
4. Evaluar la apertura de las políticas RLS orientadas a usuario final bajo el issue [#196](https://github.com/reyduar/Vaqcrow/issues/196) antes de que #18/#24/#26 expongan las tablas a un rol que no sea `service_role`.

### Corrección posterior — 2026-09-20

> Al auditar el cierre de esta Feature se corrigieron **tres defectos de exactitud** en este documento. Ninguno cambia lo que se probó, lo que quedó implementado ni los resultados de la §4: la evidencia de esas corridas sigue siendo la de los días en que cada Task se cerró. Los tres son de puntería y de vigencia.
>
> 1. **Puntero roto.** La §5 (límite 2), la §9 y el §10 (paso 4) diferían las políticas RLS a [#134](https://github.com/reyduar/Vaqcrow/issues/134), que trata sobre Auth.js y límites de sesión —no sobre RLS— y que además estaba cerrado sin implementación. El seguimiento quedaba **huérfano**: una búsqueda por "RLS" en todos los issues, abiertos y cerrados, no devolvía nada. Lo trackea [#196](https://github.com/reyduar/Vaqcrow/issues/196) desde el 20/09/2026.
> 2. **Alcance subestimado.** La §5 (límite 2) nombraba una sola tabla. Verificado contra el proyecto en vivo el 20/09/2026 (`pg_class`, `pg_policies` y el advisor de seguridad), **`human_decision` tiene exactamente la misma forma** —RLS habilitada, cero políticas, y solo `postgres` y `service_role` con GRANT—, y llegó después desde una Feature distinta ([#19](https://github.com/reyduar/Vaqcrow/issues/19)). Son dos tablas, no una.
> 3. **La §5 (límite 4) había dejado de ser cierta.** Afirmaba que el repositorio no tiene CI de ningún tipo. `ci.yml` existe desde que lo agregó [#15](https://github.com/reyduar/Vaqcrow/issues/15). Se reformuló al riesgo que **sí** sigue vigente: la suite de integración no aparece en ese workflow, así que nadie la corre automáticamente.
