...
- **GitHub y estado:** [issue #21](https://github.com/reyduar/Vaqcrow/issues/21) · Tipo `Feature` · Área `ai` · Prioridad `Critical` · Workflow `Ready`.
- **Jerarquía y bloqueos:** padre [#6](#^issue-6); bloqueada nativamente por [#20](#^issue-20), ya completo — sin bloqueos propios pendientes.
- **Objetivo:** invocar un adaptador independiente del proveedor y persistir metadatos de modelo, prompt y versión.
- **Orden:** es la unidad `Ready` de mayor prioridad (`Critical`) y desbloquea [#22](#^issue-22).
- **Proveedor:** `DEMO.md` deja el proveedor LLM real como `TBD`; sus criterios de aceptación (adaptador reemplazable, timeout/error tipado, solo la evidencia suministrada) se prueban con un proveedor falso, así que el `TBD` no bloquea la implementación, solo el ensayo en vivo.
