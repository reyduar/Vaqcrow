...
- [x] **T4 — Recorrido live opt-in.** `e2e-live/` + config propia + `test:e2e:live` contra la API docker y Quickstart; ejecutado por el padre. Ruta: writer delegado + verificación inline.
...
- Resultado live (2 corridas del writer, contenedor de la API con la imagen anterior): (a) apertura por UI con creación de la cuenta de la PyME verificada en Horizon — pasa; (b) aporte/retiro, (c) meta → `settled`, (d) reembolso tras el plazo — fallan por el bug 1 (el contenedor no incluye el arreglo). Tras reconstruir el contenedor (`pnpm env:docker:up`, ejecutado por el usuario), el padre corrió `pnpm --filter @vaqcrow/web test:e2e:live`: **4/4** — (a) apertura + cuenta de la PyME en Horizon 5,6 s; (b) aporte y retiro reflejados en la cadena 15,2 s; (c) meta completa → `settled` y pago a la PyME 10,2 s; (d) primer reembolso sin permisos disparado por otra wallet tras el plazo 35,6 s. Confirma ambos arreglos contra la cadena real.
